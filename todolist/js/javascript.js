...
if (localStorage.arrayForIndex) {
  ...
  for (let ii, i = 0; i < listItem.length; i++, ii = ii + 2) {
    ...
    arrayForSort.push({
      indexOfLi: JSON.parse(localStorage.arrayForIndex)[i],
      someLi: listItem[i]
    });
  }
...
function createNewElem(task) {
  ...
  var formatDateToday = new Date(dateToday.getFullYear(), dateToday.getMonth(),
    dateToday.getDate(), dateToday.getHours() + 3, dateToday.getMinutes());
  ...
  buttonImportant.innerHTML =
    '<button class="buttonImportant__up">&#9650;</button><button class="buttonImportant__down">&#9660;</button>';
  ...
  functButton.innerHTML =
    '<button class="functionButton__edit functionButtonCirkle"><i><img src="img/pencil.png"></i></button><button class="functionButton__done functionButtonCirkle"><i><img src="img/checked.png"></i></button><button class="functionButton__remove functionButtonCirkle"><i><img src="img/delete.png"></i>';
...
function filterArrByDate() {
  ...
  if (arrayForSort.length > 1) {
    ...
    taskList.innerHTML =
      '<h1 id="noTask" style="display:none;">PLEASE ADD TASKs</h1>';
...
function doneTask() {
  ...
  listItem.classList.toggle('toggleClass');
  buttonBg.classList.toggle('toggleClass');
  saveToLStorage();
}
...
function filterByText() {
  if (arrayForSort.length > 0) {
    ...
    if (textForSort) {
      dimmingBlockH.style.display = "block";
    } else {
      dimmingBlockH.style.display = "none";
    }
    ...
    if (counterForIndex * (-1) == li.length) concernsNot.style.display =
      "block";
